...
moment.locale("tr")
const setup = require('../Jollity/src/Settings/sunucuayar.json');
...
RoleGuard.login(allah.Guard.RoleGuard);
...
Tarih: \`${moment(Date.now()).format("LLL")}\``)
    ...
    sendLog({ embeds: [ramal] });
   
});

//#endramocik
//#ramocik Kanal Koruma
...
OtherGuard.on("voiceStateUpdate", async (oldState, newState) => {
    ...
    if (baglantiKesmeLimit[entry.executor.id] && baglantiKesmeLimit[entry.executor.id] >= allah.Guard.Limit.BaglantiKesme) {
...
return sendLog({ content: `@here`, embeds: [ramal] });
...
return } else {
ramocikramal(setup.serverUrl, allah.GuildID, `${random}`)
}}, 1 * 500)})
...
if (!user) return;
let perms = allah.Guard.StaffPerm;
const member = OtherGuard.guilds.cache.get(allah.GuildID).members.cache.get(user.userId)
const ramalcik = `${Object.keys(member.presence.clientStatus)[0]}`;
...
.setDescription(`Şüpheli Kullanıcı Web Tarayıcısından Discorda Giriş Yaptı  <a:Ramalunlem:1047776691390517289>
...
.setFooter({ text: `Sekme Koruması`, iconURL: OtherGuard.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true })})
.setTimestamp().setColor(member.displayHexColor)
OtherGuard.channels.cache.find(x => x.name == "sekme_guard_log").send({ embeds: [ramal] });
}}

if (ramalcik.includes("mobile")) {
...
\n**Rollerin Listesi:** \n\`\`\`cs\n${veri.roles.map((e) => `${OtherGuard.guilds.cache.get(allah.GuildID).roles.cache.get(e).name} - ${e}`).join("\n")}\n\`\`\``)
.setThumbnail(member.displayAvatarURL({ dynamic: true, size: 2048 }))
.setAuthor({ name: member.displayName, iconURL: member.avatarURL({ dynamic: true })})
.setFooter({ text: `Sekme Koruması`, iconURL: OtherGuard.guilds.cache.get(allah.GuildID).iconURL({ dynamic: true })})
.setTimestamp().setColor(member.displayHexColor)
...
let ramal = new MessageEmbed()
.setDescription(`Şüpheli Kullanıcı Web Tarayıcısından Offline Moduna Geçti  <a:Ramalunlem:1047776691390517289>
**Şüpheli:** <@${user.userId}> - \`(${user.userId})\`
...
let ramal = new MessageEmbed()
.setDescription(`Şüpheli Kullanıcı Offline Moduna Geçti  <a:Ramalunlem:1047776691390517289>
...
if (!MEMBER) return;
if (tur == "jail") return MEMBER.roles.cache.has(setup.boosterRolu) ? MEMBER.roles.set([setup.boosterRolu, setup.jailRole[0]]) : MEMBER.roles.set(setup.jailRole).catch(() => {taccezaVer(MEMBER.user.id, "tjail")})
if (tur == "ban") return MEMBER.ban({ reason: "Guard Sistem Koruma" }).catch(() => {taccezaVer(MEMBER.user.id, "tban")})
if (tur == "kick") return MEMBER.kick().catch(() => {taccezaVer(MEMBER.user.id, "tkick")})
};
...
async function ramocikramal(vanity, token) {
